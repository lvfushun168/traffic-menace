...
func _init() -> void:
	...
	print("GRAYBOX_MERGE_GEOMETRY_TEST edges=%s endpoints=%s monotonic=%s no_cross_road=%s completed=%s no_junction=%s dynamic_spawn=%s dynamic_sync=%s recycled=%s left=%s right=%s" % [edge_lanes_ok, endpoints_ok, monotonic_ok, no_cross_road_ok, completed_ok, no_junction_without_car, dynamic_spawn_ok, dynamic_sync_ok, recycled_ok, left_result, right_result])
	quit(0 if edge_lanes_ok and endpoints_ok and monotonic_ok and no_cross_road_ok and completed_ok and no_junction_without_car and dynamic_spawn_ok and dynamic_sync_ok and recycled_ok else 1)
